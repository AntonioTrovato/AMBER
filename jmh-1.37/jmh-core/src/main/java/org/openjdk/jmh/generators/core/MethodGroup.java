...
class MethodGroup implements Comparable<MethodGroup> {
    ...
    public Optional<String> getDynamicHaltHost() {
        for (DynamicHalt ann : getAll(DynamicHalt.class))
            if (ann.host() != DynamicHalt.HOST)
                return Optional.of(ann.host());
        // if it is not present, return none Optional
        return Optional.none();
    }

    public Optional<String> getDynamicHaltPort() {
        for (DynamicHalt ann : getAll(DynamicHalt.class))
            if (ann.port() != DynamicHalt.PORT)
                return Optional.of(ann.port());
        // if it is not present, return none Optional
        return Optional.none();
    }
    ...
    public Optional<String> getDynamicHaltModel() {
        for (DynamicHalt ann : getAll(DynamicHalt.class))
            if (ann.port() != DynamicHalt.MODEL)
                return Optional.of(ann.model());
        // if it is not present, return none Optional
        return Optional.none();
    }
