...
public class CommandLineOptions implements Options {
    ...
    private final Optional<String> dynamicHaltHost;
    private final Optional<String> dynamicHaltPort;
    private final Optional<String> dynamicHaltModel;
    ...
    public CommandLineOptions(String... argv) throws CommandLineOptionException {
        ...
        OptionSpec<String> optHaltHost = parser.accepts("hhost", "Define the API host for dynamic halt. " +
                "Set it to communicate with the framework." +
                "(default: " + Defaults.DYNAMIC_HALT_HOST + ")")
                .withRequiredArg().ofType(String.class).describedAs("halt_host");

        OptionSpec<String> optHaltPort = parser.accepts("hport", "Define the API port for dynamic halt. " +
                "Set it to communicate with the framework." +
                "(default: " + Defaults.DYNAMIC_HALT_PORT + ")")
                .withRequiredArg().ofType(String.class).describedAs("halt_port");

        OptionSpec<String> optHaltModel = parser.accepts("hmodel","Define the API model for dynamic halt. " +
                "Set it to define the model that the framework has to use for predictions. " +
                "(default: " + Defaults.DYNAMIC_HALT_MODEL + ")")
                .withRequiredArg().ofType(String.class).describedAs("halt_model");
        ...
        try {
            ...
            dynamicHaltHost = toOptional(optHaltHost, set);
            dynamicHaltPort = toOptional(optHaltPort, set);
            dynamicHaltModel = toOptional(optHaltModel, set);
    ...
    @Override
    public Optional<String> getDynamicHaltHost() {
        return dynamicHaltHost;
    }

    @Override
    public Optional<String> getDynamicHaltPort() {
        return dynamicHaltPort;
    }

    @Override
    public Optional<String> getDynamicHaltModel() {return dynamicHaltModel;}
