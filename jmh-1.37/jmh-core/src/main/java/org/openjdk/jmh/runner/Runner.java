...
public class Runner extends BaseRunner {
    ...
    private BenchmarkParams newBenchmarkParams(BenchmarkListEntry benchmark, ActionMode mode) {
        ...
        //System.out.println("Dynamic Halt Host: " + benchmark.getDynamicHaltHost());
        String dynamicHaltHost = options.getDynamicHaltHost().orElse(
                benchmark.getDynamicHaltHost().orElse(
                        Defaults.DYNAMIC_HALT_HOST));

        //System.out.println("Dynamic Halt Port: " + benchmark.getDynamicHaltPort());
        String dynamicHaltPort = options.getDynamicHaltPort().orElse(
                benchmark.getDynamicHaltPort().orElse(
                        Defaults.DYNAMIC_HALT_PORT));

        String dynamicHaltModel = options.getDynamicHaltModel().orElse(
                benchmark.getDynamicHaltModel().orElse(
                        Defaults.DYNAMIC_HALT_MODEL));
